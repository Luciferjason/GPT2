...
class SwiGLU(nn.Module):
    def __init__(self, input_dim, output_dim):
        super().__init__()
        self.W1 = torch.randn(input_dim, output_dim, requires_grad=True)  # W1 权重矩阵
        self.W2 = torch.randn(input_dim, output_dim, requires_grad=True)  # W2 权重矩阵
        self.b1 = torch.randn(output_dim, requires_grad=True)  # b1 偏置向量
        self.b2 = torch.randn(output_dim, requires_grad=True)  # b2 偏置向量
    def forward(self, x):
        # 线性变换加Swish激活
        z1 = torch.matmul(x, self.W1) + self.b1
        swish_z1 = swish(z1)
        # 线性变换加GLU激活
        z2 = torch.matmul(x, self.W2) + self.b2
        return swish_z1 * sigmoid(z2)
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 加入SwiGLU层
        self.swiglu = SwiGLU(4 * config.n_embd, 4 * config.n_embd)
